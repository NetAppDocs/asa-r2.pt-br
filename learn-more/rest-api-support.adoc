...
Existem diversas diferenças entre a ASA r2 API REST e a API REST unificada do ONTAP disponível nos sistemas FAS, AFF e ASA. Compreender essas mudanças ajudará você a usar a documentação de referência da API online com mais eficiência.
...
Algumas combinações de endpoint e método do ASA r2 não suportam todas as propriedades definidas para a personalidade unificada do ONTAP. Por exemplo, ao usar PATCH com o endpoint `/api/storage/volumes/{uuid}`, diversas propriedades não são suportadas pelo ASA r2, incluindo:
...
O OAuth 2.0 é o padrão do setor de autorização. Ele é usado para restringir e controlar o acesso a recursos protegidos com base em tokens de acesso assinados. Você pode configurar o OAuth 2.0 usando o System Manager para proteger os recursos do sistema ASA r2.
