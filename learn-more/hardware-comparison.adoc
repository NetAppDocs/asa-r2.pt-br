...
searchtitle: Diferenças entre o ASA r2 e outros sistemas ONTAP 
---
= Diferenças entre o ASA r2 e outros sistemas ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Os sistemas ASA r2 são soluções de hardware e software para ambientes exclusivamente SAN, construídas sobre sistemas all-flash. Eles diferem de outros sistemas ONTAP (ASA, AFF e FAS) em personalidade ONTAP, camada de storage e protocolos suportados.
...
Em um sistema ASA r2, o software ONTAP é otimizado para fornecer suporte às funcionalidades essenciais de SAN, limitando a visibilidade e a disponibilidade de recursos e funções não relacionados a SAN. Por exemplo, o System Manager em execução em um sistema ASA r2 não exibe opções para criar diretórios home para clientes NAS. Essa versão otimizada do ONTAP é identificada como a _personalidade ASA r2_. O ONTAP em execução em sistemas ASA é identificado como a _personalidade ASA ONTAP_. O ONTAP em execução em sistemas AFF e FAS ONTAP é identificado como a _personalidade ONTAP unificada_. As diferenças entre as personalidades do ONTAP são mencionadas na referência de comandos do ONTAP (páginas de manual), na especificação da API REST e nas mensagens EMS, quando aplicável.
...
A camada de storage dos sistemas FAS, AFF e ASA utiliza agregados como unidade básica de storage. Um agregado possui um conjunto específico de discos disponíveis em um sistema de storage. O agregado aloca espaço nos discos que possui para volumes, LUNs e namespaces. Com esses sistemas, os usuários do ONTAP podem criar e modificar agregados, volumes, LUNs e namespaces.
...
Quando uma unidade de armazenamento (baseada em um LUN ou em um namespace NVMe) é criada, ONTAP cria automaticamente um volume na zona de disponibilidade de storage apropriada para abrigar a unidade de armazenamento. O volume recém-criado é automaticamente posicionado dentro da zona de disponibilidade de storage para desempenho ideal e utilização equilibrada da capacidade. A utilização da capacidade é equilibrada dentro da zona de disponibilidade de storage com base na sua versão do ONTAP. link:../administer/space-balancing.html["Aprenda sobre balanceamento de capacidade em um cluster ASA r2"^].
...
Os novos clusters contêm automaticamente uma storage VM de dados padrão com protocolos SAN habilitados. As LIFs de dados IP suportam os protocolos iSCSI e NVMe/TCP e usam a  `default-data-blocks` política de serviço por padrão.
