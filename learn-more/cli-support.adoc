...
[role="lead"]
Os sistemas ASA r2 usam zonas de disponibilidade de storage em vez de agregados, portanto, alguns comandos e parâmetros da CLI do ONTAP não são compatíveis. Você também pode usar a CLI para configurar um cluster se preferir em vez do System Manager.
...
A menos que esteja listado como não suportado, todos os outros comandos link:https://docs.netapp.com/us-en/ontap-cli/["Referência do comando ONTAP"] são suportados para sistemas ASA r2.

Os seguintes comandos e conjuntos de comandos não são suportados no ASA r2:
...
É recomendável que você link:../install-setup/initialize-ontap-cluster.html["Use o Gerenciador do sistema para configurar o cluster do ONTAP ASA R2"]. O System Manager oferece um fluxo de trabalho guiado rápido e fácil para colocar seu cluster em funcionamento. No entanto, se você estiver acostumado a trabalhar com comandos do ONTAP, a interface de linha de comando do ONTAP (CLI) pode ser usada opcionalmente para a configuração do cluster. Configurar o cluster com a CLI não oferece opções ou vantagens adicionais em relação ao System Manager.

Durante a configuração do cluster, o ONTAP cria sua VM de storage de dados padrão, cria uma unidade de storage inicial e descobre automaticamente suas LIFs de dados. Opcionalmente, você pode habilitar o Domain Name System (DNS) para resolver nomes de host, configurar seu cluster para usar o Network Time Protocol (NTP) para sincronização de tempo e habilitar a criptografia de dados em repouso.
