...
[role="lead"]
Use o System Manager para configurar o acesso ao controlador de domínio do Active Directory, autenticação LDAP e SAML para suas contas de usuário. Crie funções de conta de usuário para definir funções específicas que os usuários atribuídos a essas funções podem executar em seu cluster.



[[configure-active-directory-domain-controller-access]]
== Configurar o acesso do controlador de domínio do Active Directory

Configure o acesso do controlador de domínio do Active Directory (AD) ao seu cluster ou máquina virtual de armazenamento para que você possa habilitar o acesso à conta do AD.
...
.Passos
. No System Manager, selecione *Cluster > Settings*.
. Em *Segurança*, ao lado de *autenticação SAML*, selecione image:icon_gear_white_bg.png["contorno azul de uma engrenagem mecânica"].
...
. Para ativar a MFA, selecione image:icon_add.gif["sinal de adição seguido pela palavra adicionar em letras azuis"]; em seguida, selecione um método de login secundário e método de autenticação.
