...
Se ocorrer uma falha de componente, uma LIF pode assumir ou ser migrada para outra porta física, para que a comunicação de rede não seja interrompida. A partir do ONTAP 9.19.1, a transferência automática de LIF é suportada por padrão para LIFs iSCSI, NVMe/TCP e IP compartilhadas. Quando a transferência automática de LIF está habilitada, se ocorrer uma falha de armazenamento, a LIF é migrada automaticamente de seu nó ou porta de origem para o nó ou porta do parceiro de HA e, em seguida, retorna assim que a transferência for concluída. Ou, se a porta ficar inativa, a LIF é migrada automaticamente para uma porta ativa em seu nó de origem atual e, em seguida, retorna à sua porta original assim que a porta estiver ativa novamente.

No ONTAP 9.18.1 e versões anteriores, o failover automático de LIF não é compatível com LIFs NVMe/TCP ou LIFs de IP compartilhado. Ao criar LIFs somente iSCSI, o failover automático de LIF é ativado por padrão, a menos que a VM de armazenamento já possua LIFs iSCSI com failover automático desativado. Se houver LIFs somente iSCSI na VM de armazenamento com failover automático desativado, as LIFs recém-criadas também não terão o failover automático ativado. Nesses casos, você deve primeiro excluir as LIFs somente iSCSI existentes e recriá-las com failover automático de LIF ativado. Saiba como link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["Excluir uma LIF em um ambiente SAN"^].
...
No ONTAP 9.18.1 e versões anteriores, o failover automático de LIF é habilitado por padrão quando você cria LIFs somente iSCSI, se todas as outras LIFs iSCSI na VM de armazenamento também estiverem habilitadas para failover automático de LIF. Se houver LIFs somente iSCSI na VM de armazenamento que não estejam habilitadas para failover automático de LIF, as LIFs recém-criadas também não serão habilitadas para failover automático de LIF.
