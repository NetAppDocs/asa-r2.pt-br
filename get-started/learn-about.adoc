...
Os sistemas ASA r2 suportam todos os protocolos SAN (iSCSI, FC, NVMe/FC, NVMe/TCP). Todos os protocolos suportam arquitetura ativo-ativo simétrica para multipath, de forma que todos os caminhos entre os hosts e o armazenamento estejam ativos/otimizados. Os protocolos iSCSI e NVMe/TCP suportam configurações de rede e de conexão direta para todas as versões do ONTAP suportadas. Os protocolos FC e NVMe/FC suportam configurações de rede para todas as versões do ONTAP suportadas e, a partir do ONTAP 9.19.1, também suportam configuração de conexão direta.
