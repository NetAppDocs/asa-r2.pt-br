...
. Selecione *Iniciar cluster remoto*; isso gera uma senha que você usará para autenticar com o cluster remoto.
...
.O que se segue?
Você estabeleceu o emparelhamento do seu ASA r2 local com um cluster remoto. Agora você pode criar uma relação de replicação.
...
A política de replicação define quando as atualizações realizadas no cluster ASA r2 são replicadas para o local remoto. O ONTAP inclui várias políticas de proteção de dados predefinidas que você pode usar para seus relacionamentos de replicação. Se as políticas predefinidas não atenderem às suas necessidades, você pode criar uma política de replicação personalizada.
...
. Para o *Tipo de política*, selecione *Assíncrono*.
+

NOTE: Com a política assíncrona, os dados são copiados para o site remoto depois de serem gravados na origem. A replicação síncrona não é suportada para sistemas ASA r2.
...
Para maior segurança contra ataques de ransomware, crie uma relação de replicação SnapLock for SnapVault.
...
=== Crie um relacionamento para replicar snapshots em um cluster remoto

Proteja seus dados replicando snapshots para um cluster remoto. Snapshots replicados de seus grupos de consistência podem ser usados para recuperação de desastres ou para migração de dados.
...
.. Procure e selecione o grupo de consistência de origem.
.. Em *destino de replicação*, selecione o cluster para o qual você deseja replicar e, em seguida, selecione a VM de armazenamento.


|===
. Selecione uma política de replicação.
. Para atrasar o início da transferência de dados, selecione *Configurações de destino* e, em seguida, desmarque *Iniciar transferência imediatamente*.
+
A transferência inicial de dados começa imediatamente por padrão.

. Opcionalmente, para substituir a programação de transferência padrão, selecione *Configurações de destino* e, em seguida, selecione *Substituir programação de transferência*.
+
...
.O que se segue?
Agora que você criou uma política e um relacionamento de replicação, sua transferência de dados inicial começa conforme definido na sua política de replicação. Opcionalmente, você pode testar o failover de replicação para verificar se o failover ocorre com sucesso caso o sistema ASA r2 fique offline.



[[create-a-relationship-to-replicate-snapshots-with-snaplock-for-snapvault]]
=== Crie um relacionamento para replicar snapshots com SnapLock for SnapVault

Para proteção contra ataques de ransomware, ao configurar sua relação de replicação, você pode habilitar o SnapLock for SnapVault selecionando uma política de replicação do cofre e, em seguida, selecionando a opção para bloquear os snapshots de destino. Os snapshots protegidos pelo SnapLock for SnapVault não podem ser excluídos acidentalmente ou maliciosamente.


NOTE: Os sistemas ASA r2 não suportam o modo SnapLock Compliance nem o modo SnapLock Enterprise. Criar uma relação de replicação SnapLock for SnapVault só permite o bloqueio dos snapshots de destino. Isso não habilita o modo SnapLock Compliance nem o modo Enterprise.

.Antes de começar
Você deve link:../secure-data/snaplock.html#initialize-the-snaplock-compliance-clock["Inicialize o SnapLock Compliance Clock"].

.Passos
. No System Manager, selecione *Proteção > Grupos de consistência* e, em seguida, selecione um grupo de consistência.
. Selecione image:icon_kabob.gif["três pontos azuis verticais"], e depois selecione *Proteger*.
...
. Selecione *Configurações de destino* e, em seguida, selecione *Bloquear snapshots de destino para impedir a exclusão*.
...
.O que se segue?
Agora que você criou uma relação de replicação, sua transferência de dados inicial começa conforme definido em sua política de replicação. Opcionalmente, você pode testar o failover da replicação para verificar se o failover ocorre com sucesso caso seu sistema ASA r2 fique offline.
