...
---
= SnapMirror fluxo de trabalho de configuração do active sync para sistemas ASA r2
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Embora os procedimentos do System Manager para configurar o SnapMirror active sync sejam diferentes em sistemas ASA r2 do que em sistemas NetApp FAS, AFF e ASA executando a personalidade unificada do ONTAP, os requisitos, a arquitetura e a operação do SnapMirror active sync são os mesmos. link:https://www.netapp.com/pdf.html?item=/media/138366-sb-3457-san-disaster-recovery-netapp-asa.pdf["Saiba mais sobre recuperação de desastres com sincronização ativa SnapMirror no seu sistema ASA r2"^]

Nos sistemas ASA r2, SnapMirror active sync oferece suporte a configurações ativo-ativo simétricas. Em uma configuração ativo-ativo simétrica, ambos os sites podem acessar o storage local para I/O ativo. Saiba mais sobre link:https://docs.netapp.com/us-en/ontap/snapmirror-active-sync/architecture-concept.html#symmetric-activeactive["configurações simétricas ativas/ativas"^].

Os modos SnapLock Enterprise e Compliance não são suportados com o SnapMirror active sync em sistemas ASA r2.

.Sobre esta tarefa
* A partir do ONTAP 9.19.1, a Proteção Autônoma contra Ransomware com Inteligência Artificial (ARP/AI) oferece suporte a unidades de storage primário de leitura e gravação em relacionamentos SAN de sincronização ativa do SnapMirror em sistemas ASA r2. O ARP/AI é executado nas unidades de storage primário ativas, cria snapshots ARP apenas para o primário e fornece detecção de ransomware baseada em aprendizado de máquina para essas unidades de storage. Para as etapas de capacitação do ARP/AI, consulte link:../secure-data/enable-anti-ransomware-protection.html["Habilite a proteção autônoma contra ransomware com IA"].
...
.Antes de começar
Se você estiver executando ONTAP 9.19.1 ou posterior e habilitando SnapMirror active sync para um host AIX, você deve link:prepare-aix-snapmirror-active-sync.html["prepare o host AIX"].
