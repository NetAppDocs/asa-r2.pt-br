...
A partir do ONTAP 9.18.1, você pode definir a porcentagem de reserva de snapshots e habilitar a exclusão automática de snapshots ao criar um grupo de consistência com novas unidades de storage. A reserva de snapshots é a quantidade de espaço na unidade de storage reservada especificamente para snapshots. Quando a reserva de snapshots é configurada com a exclusão automática de snapshots, os snapshots mais antigos são excluídos automaticamente quando o espaço usado por snapshots excede a reserva de snapshots. Se a reserva de snapshots e a exclusão automática de snapshots estiverem habilitadas em um grupo de consistência pai, elas serão habilitadas em todos os grupos de consistência filhos existentes. Se novos grupos de consistência filhos forem adicionados, eles não herdarão as configurações de reserva de snapshots e exclusão de snapshots do grupo pai.

link:snapshot-reserve.html["Saiba mais sobre a reserva de snapshots em sistemas de armazenamento ASA r2."].

A partir do ONTAP 9.16.1, ao criar grupos de consistência usando novas unidades de storage, você pode configurar até cinco grupos de consistência filhos. link:manage-hierarchical-consistency-groups.html["Saiba mais sobre grupos de consistência infantil em sistemas ASA r2"].
...
====
.O que se segue?
Agora que seus dados estão protegidos com snapshots, você deve link:snapshot-replication.html["configurar a replicação de instantâneos"] copiar seus grupos de consistência para um local geograficamente remoto para backup e recuperação de desastres.
