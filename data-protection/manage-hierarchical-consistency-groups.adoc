...
summary: Utilize grupos de consistência para executar diversas ações em várias unidades de storage ao mesmo tempo, em vez de executar ações individualmente em cada unidade. Em uma estrutura hierárquica, um ou mais grupos de consistência são configurados como filhos sob um grupo de consistência pai. 
---
...
[role="lead"]
A partir do ONTAP 9.16.1, você pode usar o System Manager para criar grupos de consistência hierárquicos no seu sistema ASA r2. Em uma estrutura hierárquica, um ou mais grupos de consistência são configurados como filhos sob um grupo de consistência pai. Você pode aplicar políticas de snapshot individuais a cada grupo de consistência filho e replicar os snapshots de todos os grupos de consistência filhos para um cluster remoto como uma única unidade, replicando o grupo pai. Isso simplifica a proteção e o gerenciamento de dados para estruturas de dados complexas.
...
. Passe o cursor sobre o grupo de consistência que você deseja converter em um grupo de consistência pai.
...
.O que se segue?
Você pode criar grupos de consistência filhos adicionais sob o grupo de consistência pai. Você também pode link:snapshot-replication.html["configurar a replicação de instantâneos"] copiar os grupos de consistência pai e filhos para um local geograficamente remoto para backup e recuperação de desastres.
...
. Passe o cursor sobre o grupo de consistência filho que você deseja desanexar.
