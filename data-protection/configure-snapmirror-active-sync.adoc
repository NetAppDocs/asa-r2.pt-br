...
NOTE: A partir do ONTAP 9.19.1 RC, a Proteção Autônoma contra Ransomware com Inteligência Artificial (ARP/AI) oferece suporte a unidades de storage primário de leitura e gravação em SnapMirror relacionamentos SAN de sincronização ativa em sistemas ASA r2. O ARP/AI é executado nas unidades de storage primário ativas, cria snapshots ARP somente primários e fornece detecção de ransomware baseada em aprendizado de máquina para essas unidades de storage. Para habilitar o ARP/AI, consulte link:../secure-data/enable-anti-ransomware-protection.html["Habilite a proteção autônoma contra ransomware com IA"].
