...
[role="lead"]
Utilize o ONTAP System Manager para monitorar o desempenho geral do seu cluster e de unidades de storage específicas, incluindo latência, IOPS e taxa de transferência, em períodos que variam de uma hora a um ano.
...
Utilize métricas de desempenho da unidade de storage para determinar o impacto de aplicações específicas na latência, IOPS e taxa de transferência.
...
.O que se segue?
Utilize as métricas de desempenho da sua unidade de storage para analisar se você precisa modificar as políticas de QoS atribuídas às suas unidades de storage para diminuir a latência e maximizar as IOPS e a taxa de transferência.
