...
.O que se segue?
Se o seu cluster estiver com pouco espaço ou não tiver capacidade para atender à demanda futura, link:../administer/increase-storage-capacity.html["adicionar novas unidades"] ao seu sistema ASA r2 para aumentar a sua capacidade de storage.
...
Cada par de HA em um sistema ASA r2 utiliza um pool de storage comum chamado _zona de disponibilidade de storage_. A zona de disponibilidade de storage tem acesso a todos os discos disponíveis no sistema de storage e é visível para ambos os nós do par de HA. Monitorar o espaço disponível das suas zonas de disponibilidade de storage permite diminuir o uso ou aumentar a capacidade conforme necessário, evitando falhas nas cargas de trabalho devido à falta de espaço.
...
A partir do ONTAP 9.19.1, um alerta é exibido em *Insights* do System Manager se uma zona de disponibilidade de storage em um dos seus pares de HA do sistema ASA r2 estiver próxima da capacidade. Saiba mais sobre System Manager link:view-insights.html["Percepções"].
...
+
Em *Visão geral*, a capacidade da zona de disponibilidade de storage, a quantidade de espaço utilizado e a taxa de redução de dados são exibidas.

+
Em *Unidades de armazenamento*, é exibida uma lista de todas as unidades de armazenamento na zona de disponibilidade de storage.



.O que se segue?
Se a sua zona de disponibilidade de storage estiver com pouco espaço, link:../manage-data/move-storage-units.html["mova as unidades de armazenamento"] para outra zona de disponibilidade de storage para equilibrar a utilização do storage em todo o cluster, exclua snapshots para liberar espaço ou adicione discos adicionais aos seus nós para aumentar a capacidade.
