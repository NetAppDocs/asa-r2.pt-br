...
[role="lead"]
Quando criptografados, os dados em repouso não podem ser lidos se uma mídia de armazenamento for reutilizada, devolvida, extraviada ou roubada. Você pode usar o ONTAP System Manager para aplicar criptografia de hardware e software para proteção em duas camadas.
...
NOTE: Nos sistemas ASA r2, os SEDs são suportados apenas para SSDs baseados em NVMe.
...
Além da NSE, você também pode habilitar a criptografia por software para adicionar outra camada de segurança aos seus dados. Os sistemas ASA r2 usam a tecnologia NetApp Volume Encryption (NVE) para criptografar dados no nível do software. NetApp Aggregate Encryption (NAE) não é compatível com os sistemas ASA r2 porque a camada de storage nesses sistemas ASA r2 usa storage availability zones em vez de aggregates.

Saiba mais sobre link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/configure-netapp-volume-encryption-concept.html["Criptografia de volume NetApp"^].

Saiba mais sobre link:https://docs.netapp.com/us-en/asa-r2/learn-more/hardware-comparison.html["as diferenças entre os sistemas ASA r2 e outros sistemas ONTAP"].
