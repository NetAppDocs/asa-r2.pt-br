...
| link:../data-protection/manage-consistency-groups.html["Suporte para alterações de geometria em grupos de consistência em relacionamentos de replicação"] | Os sistemas ASA r2 suportam alterações de geometria em grupos de consistência em uma relação de sincronização ativa do SnapMirror ou em uma relação de replicação assíncrona, sem excluir a relação de sincronização ativa do SnapMirror ou interromper a relação assíncrona. Quando ocorre uma alteração de geometria no grupo de consistência primário, a alteração é replicada para o grupo de consistência secundário. 
