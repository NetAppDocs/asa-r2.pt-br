...
[role="lead"]
A clonagem de dados cria cópias de unidades de storage e grupos de consistência no seu ASA r2. Você pode usar as cópias para desenvolvimento de aplicativos, testes, backups, migração de dados e outras funções administrativas.
...
.. Insira um nome para o novo host; em seguida, selecione *FC* ou *iSCSI*.
...
.Passos
. No System Manager, selecione *proteção > grupos de consistência*.
. Passe o cursor sobre o clone do grupo de consistência que você deseja dividir.
